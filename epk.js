...
function buildEPK(epk) {
  ...
  const featuredKeys = ['website','spotify','youtube','instagram'];
  ...
  const buildLinkRow = (key) => {
    ...
    return urls.map((url, i) => {
      const followers = i === 0 ? (s[key + '_followers'] || '') : '';
  ...
  const featuredHTML = [
    ...
    buildFeaturedCard('youtube', 'YouTube', 'YouTube Channel', s.youtube_followers || ''),
